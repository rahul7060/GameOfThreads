...
const Cart = () => {
    ...
    // Update Quantity
    const updateQuantity = useCallback(async (productId, newQuantity, size) => {
        ...
        try {
            ...
            setCartItems(prevItems => 
                prevItems.map(item => 
                    item.product._id === productId && item.size === size ? { ...item, quantity: newQuantity } : item
                )
            );
    ...
    const removeFromCart = useCallback(async (productId, size) => {
      console.log(`Removing product ${productId} with size ${size}`);
      try {
          const response = await cartServices.removeFromCart(productId, size);
          if (response.error) throw new Error(response.error);
  
          setCartItems(prevItems => {
              const updatedCart = prevItems.filter(item => !(item.product._id === productId && item.size === size));
              return updatedCart;
          });
                ...
                <div>
                    {cartItems.map(item => (
                        <div key={`${item.product._id}-${item.size}`} className="flex items-center gap-4 border-b py-4">
                            ...
                            <button className="px-2 py-1 border" disabled={item.quantity <= 1} onClick={() => updateQuantity(item.product._id, item.quantity - 1, item.size)}>-</button>
